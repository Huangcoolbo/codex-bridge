...
- 主进程 HTTP route：
  [agentGatewayService.ts](/D:/remote-agent-bridge/desktop-client/src/main/services/agentGatewayService.ts)
- target 持久化与字段映射：
  [bridgeService.ts](/D:/remote-agent-bridge/desktop-client/src/main/services/bridgeService.ts)
- 命令执行与最近结果：
  [automationService.ts](/D:/remote-agent-bridge/desktop-client/src/main/services/automationService.ts)
- 当前会话目标：
  [sessionService.ts](/D:/remote-agent-bridge/desktop-client/src/main/services/sessionService.ts)
- Python bridge / 远端执行：
  [service.py](/D:/remote-agent-bridge/src/remote_agent_bridge/service.py)
...
或者进入 [desktop-client](/D:/remote-agent-bridge/desktop-client) 运行：
...
用途：新增一个 target，并写回 [hosts.json](/D:/remote-agent-bridge/data/hosts.json)。
...
用途：更新一个已有 target，并写回 [hosts.json](/D:/remote-agent-bridge/data/hosts.json)。
...
用途：删除 target，并从 [hosts.json](/D:/remote-agent-bridge/data/hosts.json) 移除。
...
- 不写入 [hosts.json](/D:/remote-agent-bridge/data/hosts.json)
- 适合临时密码输入或一次性认证覆盖

### `password`

- 只有在密码认证且 `store_password=true` 或 `storePassword=true` 时才持久化
- 如果未显式要求持久化，密码不会写入 [hosts.json](/D:/remote-agent-bridge/data/hosts.json)

### `keyPath`

- 允许通过接口设置
- 会持久化到 [hosts.json](/D:/remote-agent-bridge/data/hosts.json)

### 默认项目密钥

Windows target 默认优先使用项目内密钥：

[localhost_ed25519](/D:/remote-agent-bridge/data/ssh/localhost_ed25519)
...
会写回 [hosts.json](/D:/remote-agent-bridge/data/hosts.json) 的接口：

- `POST /api/targets`
- `PUT /api/targets/:name`
- `DELETE /api/targets/:name`

不会写回 [hosts.json](/D:/remote-agent-bridge/data/hosts.json) 的接口：
