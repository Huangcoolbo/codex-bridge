...
- [~] 能向远程 Windows 发送 PowerShell 命令（已补 UTF-8 输出与 Stop 失败语义，待真实远程验证）
...
- [>] 让桥更像“远程执行通道”而不是零散工具集合（已补 exec --cwd，开始支持远程工作目录语义）
- [>] 让 Codex 可以连续调用桥完成多步远程操作（已补 exec --cwd，下一步补更清晰的连续操作示例/闭环）
...
- [x] 补脚本：查看远程目录脚本
- [x] 补脚本：写入远程文件脚本
- [>] 继续优化错误提示，让失败原因更直白（已补 list-dir 的目录不存在/误传文件语义；已补 exec 的 Stop 失败语义与 --cwd 的工作目录不存在/误传文件语义；write-file 已补父路径/目录误写保护与写后元数据返回；下一步优先做真实远程验证）
